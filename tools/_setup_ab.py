"""A/B: BT_REQUIRE_SETUP (clean-setup-only entries) - test windows."""
...
def run_one(task):
    ...
    c.BT_REQUIRE_SETUP = bool(req)
    keep = df5["date"].dt.strftime("%Y-%m").isin(months(TEST))
    r = Backtest(c, df=df5[keep], df1m=df1, verbose=False).run()
    return {"idx": idx, "req": req, "trades": r["trades"], "win": r["win_rate"],
            "net": r["net_pnl"], "pf": r["profit_factor"], "maxdd": r["max_drawdown_pct"]}


def main():
    tasks = [(i, r) for i in ("NIFTY", "BN") for r in (0, 1)]
    print(f"clean-setup A/B on {TEST}: {len(tasks)} replays", flush=True)
    ...
    for idx in ("NIFTY", "BN"):
        ...
        for res in sorted([x for x in results if x["idx"] == idx], key=lambda x: -(x["net"] or 0)):
            ...
            print(f"  {'require-clean-setup' if res['req'] else 'base':<20} "
                  f"tr={res['trades']:>4} win={res['win']:>5.1f}% net={res['net']:>+11,.0f} "
                  f"PF={pf:>5.2f} maxDD={res['maxdd']:>5.2f}%", flush=True)
