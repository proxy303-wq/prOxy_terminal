"""The blocked trade: signal strike vs the shift, and the block message."""
import sys, os
...
for db, tag in (("proxy_state.sqlite", "NIFTY"), ("proxy_state_banknifty.sqlite", "BANKNIFTY")):
    print(f"== {tag} recent activity ==")
    out = run("python3 -c \"import sqlite3;c=sqlite3.connect('/opt/proxy/reports/%s');"
              "rows=list(c.execute(\\\"SELECT substr(ts,12,8),level,message FROM activity_log WHERE ts>='2026-09-04T14:00' ORDER BY ts DESC LIMIT 12\\\"));"
