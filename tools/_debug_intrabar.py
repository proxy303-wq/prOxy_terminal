"""Debug: why does the intra-bar stop exit return None?"""
import sys, os, types, traceback, tempfile
...
e = PaperEngine(lc, broker=types.SimpleNamespace(live=True), tracker=Tracker(lc, db_path=tmp.name),
...
e.active_trade = trade
e._active_trade = trade

bar = {"time": datetime.now(ZoneInfo("Asia/Kolkata")), "open": 94.5, "high": 94.5,
       "low": 94.5, "close": 94.5, "volume": 0.0}
try:
    out = e._check_exits(bar, None, 0.0, real_bar={"open": 94.5, "high": 94.5, "low": 94.5, "close": 94.5})
    print("direct _check_exits ->", out)
except Exception:
    traceback.print_exc()
